...
/* Aktivacni konstanty jednotlivych pozic LED displeje
 * (aktivace pomoci log. 1). */
#define D1 0x0800 // PTA11
#define D2 0x1000 // PTA12
#define D3 0x2000 // PTA13
#define D4 0x1000 // PTB12

/* Aktivacni konstanty hodnot cislic
 * (7 segmentu a-g na PTA3-PTA9, sviti v log. 0). */
#define N0 0x0607 // "0"
#define N1 0x07CF // "1"
#define N2 0x0527 // "2"
#define N3 0x0587 // "3"
...
/* Funkce display_val nastavuje vyvody LED pro spravnou aktivni pozici (index)
...
void ADC0_Init(void)
{
	// povoleni preruseni v systemu NVIC
	NVIC_ClearPendingIRQ(ADC0_IRQn);
	NVIC_EnableIRQ(ADC0_IRQn);

	// prumerovani z 32 vzorku a kontinualni konverze
	ADC0_SC3 |= 0xF;
...
int main(void)
{
	...
	while (1)
	{
		...
		delay(DELAY_LEDD);
		PTA->PDOR &= GPIO_PDOR_PDO(~D2); // deaktivace druhe cifry

		display_val(result); // cislice k zobrazeni
		PTA->PDOR |= GPIO_PDOR_PDO(D3); // aktivace treti cifry
		delay(DELAY_LEDD);
		PTA->PDOR &= GPIO_PDOR_PDO(~D3); // deaktivace treti cifry

		display_val(result); // cislice k zobrazeni
		PTB->PDOR |= GPIO_PDOR_PDO(D4); // aktivace ctvrte cifry
		delay(DELAY_LEDD);
		PTB->PDOR &= GPIO_PDOR_PDO(~D4); // deaktivace ctvrte cifry
	}
}
